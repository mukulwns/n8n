...
export class License implements LicenseProvider {
	...
	private readonly logger = Container.get(Logger).scoped('license');
	...
	async activate(_activationKey: string): Promise<void> {
		this.logger.debug('License activation bypassed for SaaS', { activationKey: _activationKey });
	}

	async renew(): Promise<void> {
		this.logger.debug('License renewal bypassed for SaaS');
	}
	...
	getManagementJwt(): string {
		return 'saas-bypass-jwt'; // Mock JWT
	}

	isLicensed(_feature: BooleanLicenseFeature): boolean {
		return true; // For LicenseState
	}

	getValue<T extends keyof FeatureReturnType>(_feature: T): FeatureReturnType[T] {
		return -1 as FeatureReturnType[T]; // Unlimited quotas
	}

	@OnPubSubEvent('reload-license')
	async reload(): Promise<void> {
		this.logger.debug('License reload bypassed for SaaS');
	}

	async clear(): Promise<void> {
		this.logger.debug('License clear bypassed for SaaS');
	}

	@OnShutdown()
	async shutdown(): Promise<void> {
		this.isShuttingDown = true;
		this.logger.debug('License shutdown bypassed for SaaS');
	}

	getCurrentEntitlements(): TEntitlement[] {
		return [this.getMainPlan()];
	}

	getConsumerId(): string {
		return 'saas-consumer';
	}
	...
	getAiCredits() {
		return this.getValue(LICENSE_QUOTAS.AI_CREDITS) ?? -1;
	}
	...
	@OnLeaderTakeover()
	enableAutoRenewals() {
		this.logger.debug('Auto renewals bypassed for SaaS');
	}

	@OnLeaderStepdown()
	disableAutoRenewals() {
		this.logger.debug('Auto renewals disable bypassed for SaaS');
	}

	private onExpirySoon() {
		this.logger.debug('License expiry warning bypassed for SaaS');
	}

	async saveCertStr(_value: TLicenseBlock): Promise<void> {
		this.logger.debug('Certificate save bypassed for SaaS');
	}
}
